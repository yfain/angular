library angular2.test.router.rules.route_paths.param_route_path_spec;
...
import "package:angular2/testing_internal.dart"
    show
        ...
        beforeEach,
        SpyObject;
import "package:angular2/src/router/rules/route_paths/param_route_path.dart"
    show ParamRoutePath;
...
main() {
  describe("PathRecognizer", () {
    it("should throw when given an invalid path", () {
      expect(() => new ParamRoutePath("/hi#")).toThrowError(
      ...
      expect(() => new ParamRoutePath("hi)")).toThrowError(
          '''Path "hi)" contains ")" which is not allowed in a route config.''');
      expect(() => new ParamRoutePath("hi//there")).toThrowError(
    ...
    describe("wildcard segment", () {
      it("should return a url path which matches the original url path", () {
        var rec = new ParamRoutePath("/wild/*everything");
        var url =
            parser.parse("/wild/super;variable=value/anotherPartAfterSlash");
        var match = rec.matchUrl(url);
        expect(match.urlPath)
            .toEqual("wild/super;variable=value/anotherPartAfterSlash");
      });
    });
  });
}
