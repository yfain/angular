library angular2.test.router.rules.rule_set_spec;
...
import "package:angular2/src/router/rules/rules.dart"
    show RouteMatch, PathMatch, RedirectMatch;
import "package:angular2/src/router/rules/rule_set.dart" show RuleSet;
...
main() {
  describe("RuleSet", () {
    RuleSet recognizer;
    beforeEach(() {
      recognizer = new RuleSet();
    });
    it(
        "should recognize a static segment",
        inject([AsyncTestCompleter], (async) {
          recognizer.config(new Route(path: "/test", component: DummyCmpA));
          recognize(recognizer, "/test").then((List<RouteMatch> solutions) {
    ...
    it(
        "should recognize a single slash",
        inject([AsyncTestCompleter], (async) {
          recognizer.config(new Route(path: "/", component: DummyCmpA));
          recognize(recognizer, "/").then((List<RouteMatch> solutions) {
            expect(solutions.length).toBe(1);
            expect(getComponentType(solutions[0])).toEqual(DummyCmpA);
            async.done();
          });
    ...
    it(
        "should recognize a dynamic segment",
        inject([AsyncTestCompleter], (async) {
          recognizer
              .config(new Route(path: "/user/:name", component: DummyCmpA));
          recognize(recognizer, "/user/brian")
              .then((List<RouteMatch> solutions) {
            expect(solutions.length).toBe(1);
            expect(getComponentType(solutions[0])).toEqual(DummyCmpA);
    ...
    it(
        "should recognize a star segment",
        inject([AsyncTestCompleter], (async) {
          recognizer
              .config(new Route(path: "/first/*rest", component: DummyCmpA));
    ...
    it(
        "should recognize a regex",
        inject([AsyncTestCompleter], (async) {
          GeneratedUrl emptySerializer(params) {
    ...
    it("should generate using a serializer", () {
      GeneratedUrl simpleSerializer(params) {
        var extra = {"c": params["c"]};
        return new GeneratedUrl(
            '''/${ params [ "a" ]}/${ params [ "b" ]}''', extra);
      }
      recognizer.config(new Route(
          name: "Route1",
          regex: "^(.+)/(.+)\$",
    ...
    describe("params", () {
      it(
          "should recognize parameters within the URL path",
          inject([AsyncTestCompleter], (async) {
            recognizer.config(new Route(
            ...
            recognize(recognizer, "/profile/matsko?comments=all")
              ...
              expect(solutions.length).toBe(1);
              expect(getParams(solutions[0]))
                  .toEqual({"name": "matsko", "comments": "all"});
              async.done();
            });
          }));
      ...
      it(
          "should prefer positional params over query params",
          inject([AsyncTestCompleter], (async) {
            recognizer.config(new Route(
                path: "profile/:name", component: DummyCmpA, name: "User"));
      ...
      it(
          "should ignore matrix params for the top-level component",
          inject([AsyncTestCompleter], (async) {
            recognizer.config(new Route(
