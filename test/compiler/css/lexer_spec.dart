library angular2.test.compiler.css.lexer_spec;
  ...
  describe("CssLexer", () {
    ...
    describe("Selector Mode", () {
      it("should throw an error if a selector is being parsed while in the wrong mode",
        ...
        expect(capturedMessage).toMatchPattern(new RegExp(
    ...
    describe("Media Query Mode", () {
      it("should validate media queries with a reduced subset of valid characters",
        ...
        expect(tokenizeQuery("(prop: value) and (prop2: value2)").length)
            .toEqual(11);
        expect(tokenizeQuery("tv and (prop: value)").length).toEqual(7);
        expect(tokenizeQuery("print and ((prop: value) or (prop2: value2))")
                .length)
            .toEqual(15);
        expect(tokenizeQuery("(content: 'something \$ crazy inside &')").length)
            .toEqual(5);
        expect(() {
