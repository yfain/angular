library angular2.test.compiler.css.parser_spec;
...
main() {
  describe("CssParser", () {
    ...
    it("should parse media queries into a stylesheet AST", () {
      ...
      assertTokens(
          rule.query, ["all", "and", "(", "max-width", ":", "100", "px", ")"]);
      var block = (rule.block as CssBlockAST);
      expect(block.entries.length).toEqual(1);
    ...
    it("should parse inline CSS values", () {
      ...
      assertTokens(
          importRule.value.tokens, ["url", "(", "remote", ".", "css", ")"]);
    ...
    it("should parse CSS values that contain functions and leave the inner function data untokenized",
      ...
      assertTokens(((defs[0] as CssDefinitionAST)).value.tokens,
          ["url", "(", "matias.css", ")"]);
      assertTokens(((defs[1] as CssDefinitionAST)).value.tokens,
          ["cubic-bezier", "(", "0.755, 0.050, 0.855, 0.060", ")"]);
      assertTokens(((defs[2] as CssDefinitionAST)).value.tokens,
          ["calc", "(", "100% - 50px", ")"]);
    });
    ...
    it("should parse multiple levels of semicolons", () {
      ...
      expect(importRule.type).toEqual(BlockType.Import);
      assertTokens(
          importRule.value.tokens, ["url", "(", "something something", ")"]);
      ...
      assertTokens(mediaQueryRule.query,
    ...
    it("should throw an error if an unknown @value block rule is parsed", () {
      ...
      expect(() {
        makeAST(styles);
          ...
          r'^CSS Parse Error: The CSS "at" rule "@matias" is not allowed to used here'));
    });
    it("should parse empty rules", () {
      ...
      var ast = makeAST(styles);
      var rules = ast.rules;
      expect(((rules[0] as CssSelectorRuleAST)).block.entries.length)
          .toEqual(0);
      expect(((rules[1] as CssSelectorRuleAST)).block.entries.length)
          .toEqual(0);
      expect(((rules[2] as CssSelectorRuleAST)).block.entries.length)
          .toEqual(1);
    });
    ...
    it("should parse the @supports rule", () {
      ...
      assertTokens(
          supportsRule.query, ["(", "animation-name", ":", "rotate", ")"]);
    ...
    it("should throw an error when parsing invalid CSS Selectors", () {
      ...
      var errors = output.errors;
      expect(errors.length).toEqual(3);
      expect(errors[0].msg).toMatchPattern(
          new RegExp(r'Unexpected character \[\[\] at column 0:7'));
      expect(errors[1].msg).toMatchPattern(
          new RegExp(r'Unexpected character \[%\] at column 0:12'));
      expect(errors[2].msg).toMatchPattern(
          new RegExp(r'Unexpected character \[}\] at column 0:19'));
    });
    ...
    it("should throw an error if a pseudo function selector is not closed properly",
      ...
      var output = parse(styles);
      var errors = output.errors;
      expect(errors[0].msg).toMatchPattern(new RegExp(
          r'Unbalanced pseudo selector function value at column 0:10'));
    });
    it("should raise an error when a semi colon is missing from a CSS style/pair that isn't the last entry",
      ...
      expect(errors[0].msg).toMatchPattern(new RegExp(
          r'The CSS key\/value definition did not end with a semicolon at column 1:15'));
    });
